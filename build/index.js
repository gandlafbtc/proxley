...
async function hasPublishedTransportAnnouncement(publicKey) {
    const filter = { kinds: [11111], authors: [publicKey] };
    const event = await ndk.fetchEvent(filter);
    event && logger("Transport announcement:", event.tags);
    return !!event;
}
async function announceTransports(publicKey) {
...
async function sendResponse(requestPubkey, convoKey, response) {
